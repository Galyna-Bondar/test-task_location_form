const detectCityLocation = () => {
    const city = document.querySelector('.input_city');
    const error_placeholder = document.querySelector('.error_placeholder');
    ...
    const translationApiUrl = `https://api.mymemory.translated.net/get?q=${principalSubdivision}&langpair=en|uk`;
    fetch(translationApiUrl)
    .then(res => res.json())
    .then(data => {
        const translatedText = data.responseData.translatedText;
        city.value = translatedText;
            //city.value = data.principalSubdivision;
        })
    })
}
    const error = () => {
...
document.querySelector(".input_city").addEventListener('click', detectCityLocation);


//Валідація щодо заборонених символів та цифр
...
function SendMail() {
    ...
    emailjs.send("service_p0xvfqj", "template_hoi39le", params)
        .then(function (res) {
            if (res.status === 200){
...
$("#form").on('submit', function(e){
 ...
 let text =encodeURI("<b>City:</b> "+$("#city").val()+"\n<b>Address:</b> "+$("#address").val());
